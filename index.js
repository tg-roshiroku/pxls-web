...
const filesInDist = listFiles(path.join("dist")).map((file) =>
    file.replace("dist/", "")
);
...
app.use("/", async (req, res, next) => {
    ...
    if (req.path === "/") {
        ...
        let dataRes;
        try {
            dataRes = await proxyFetch(
                req,
                `http://${config.proxyTo}/api/v1/profile` + profileName
            );
        } catch (err) {
        ...
        res.render("profile", {
            ...
            user: {
                ...data.user,
                signupTimeFormatted: new Date(
                    data.user.signupTime
                ).toLocaleString(),
                banExpiryFormatted: new Date(
                    data.user.banExpiry
                ).toLocaleString(),
                chatBanExpiryFormatted: new Date(
                    data.user.chatBanExpiry
                ).toLocaleString(),
            },
            isSelf: data.user.id === data.self.id,
            routeRoot: req.path,
            canvasReportsOpenCount: (data.canvasReports || []).filter(
                (r) => !r.closed
            ).length,
